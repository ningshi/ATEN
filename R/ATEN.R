...
#' @seealso \code{\link{generateRandomNKNetwork}}
#'
#' @return a set of important prime implicants
#' @examples
...
#'
#' A helper function for generating dataset in RFRE framework
#'
#' @param PIs A list of prime implicants
#' @param datalist A list of data matrices that used to update a new data list
#'
...
#'
#' @param network A network structure of class BooleanNetwork
#' @param numSeries The number of time series
#' @param numPoints The number of time points that is covered by each time series
#' @param noiseLevel The level of the nosie. e.g. 0.05 indiciates the value of a gene can be flipped with probability 0.05
#' @param wildtype a vector containing the wildtype expression values
#'
#' @return A list of data matrices containg the expression values of each node/gene. The row/column corresponds to time point/gene respectively. The second element refers to the input data and the third element refers to the respond data of the target gene.
#' @examples
#' ngenes<-10
#' ## k is the maximum number of genes
#' k<-5
...
#'
#' @return The data list after bagging. The first two elements are the in-bag data (input and respond). The last two elements are the out-of-bag data (input and respond).
#'
#' @export
...
growand<-function(tree,subtree,nodes,penalty=FALSE,dec){
  ...
  if(dec==1){
    #cat("#prune a branch by altering a leaf")
    subtree<-replace(subtree,sample(1:nleaf,1),sample(inputnodes,1))
    subtree<-sort(subtree)
  }
  else if(dec==2||penalty){
    #cat("#prune a branch by deleting a leaf")
    if(nleaf==1) return(NULL)
    subtree<-subtree[-sample(1:nleaf,1)]
    subtree<-sort(subtree)
  }
  else{
    #cat("#prune a branch by adding a new leaf")
...
#'
#' @param tree An And/Or tree
#' @param input a data matrix, each row is a time point, each column corresponds to a node/gene in the tree
#' @param nodes the number of nodes in the Boolean network
#'
#' @return a vector of the output of the tree
#' @export
...
#' @param initT the uppper temperature
#' @param endT the lower temperature
...
#' @param nodes the number of nodes in the Boolean network
#'
#' @return the difference between the output of tree1 and tree2. If a negative value is returned, then tree1 is better. If a positive value is returned, the tree2 is better. If zero is return, then tree1 and tree2 are equally good.
#'
#' @export
#'
